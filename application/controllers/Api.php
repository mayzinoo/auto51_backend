...
class Api extends CI_Controller {    
        ...
        $data = array(
          ...
          if($query->num_rows()==1)
          {
                ...
                $config = Array(       
                        ...
                        'smtp_port' => 465,
                        'smtp_user' => 'auto51testing@gmail.com',
                        'smtp_pass' => 'pweawisoelckztgg',
                        'smtp_timeout' => '4',
                        'mailtype'  => 'html',
                        'charset'   => 'iso-8859-1',
                        'wordwrap' => TRUE
                ...
                $this->email->from('auto51testing@gmail.com','Auto 51');
    ...
    public function checkbookingdate()
    {
        $return = array("status" => 0, "data" => "Invalid Booking ID");
        $_data = json_decode(file_get_contents('php://input')); 
        
        $vid=$_data->vehicle_id;
        $startdate=$_data->startDate;
        $enddate=$_data->endDate;
   
        $bookingdata = $this->db->query("SELECT * FROM bookings WHERE vehicle_id = $vid")->row();
        if($startdate>=$bookingdata->start_date){
            $return = array("status" => 1, "data" => $bookingdata->available_date);
             
        }else{
            $return = array("status" => 0, "data" => $bookingdata->available_date);
        }
        echo json_encode($return);
    }
    public function adduserbookings() {       
        ...
        $avai_date = $this->db->query("SELECT * FROM bookings WHERE vehicle_id = $cid Order By id desc")->row();
        if(empty($avai_date)){
            $bookdate=date('Y-m-d');
        }else{
            $bookdate=$avai_date->available_date;
        }
        
        $pricestatus=$_data->price_status;

        $leasingstatus=$_data->leasing_status;



        if($pricestatus=="selling" && $leasingstatus=="daily"){
        ...
        else if($leasingstatus=="daily" && $pricestatus=="leasing"){
            ...
            // $leasingstatus="daily";
            $totalrentdays=$totaldays+1;

            $month=0;
        } 
        else if($leasingstatus=="monthly" && $pricestatus=="leasing"){                
                $month=$_data->duration;
                $avi=date('Y-m-d',strtotime("+$month month",strtotime($bookdate)));
                $avilabledate=date('Y-m-d', strtotime($avi . ' +1 day'));
                $price=$_data->price;
                $total=$price*$month;

                $startdate=$_data->start_date;
                $enddate=date('Y-m-d', strtotime('-1 day', strtotime($avilabledate)));
        ...
        $data = array(
            ...
            "end_date" => $enddate,
            "total_days"=>$totalrentdays,
            "leasing_status"=>$leasingstatus,
            "total_amt"=>$total,
            "price_status"=>$pricestatus,
            "billing_city"=>$_data->billingCity,
        ...
        if($checkstatus=="false")
            {
                ...
                $avai_date = $this->db->query("SELECT * FROM bookings WHERE vehicle_id = $vehicleid AND id!=$bid Order By id desc")->row();
                if(empty($avai_date)){
                    $cardata = array(
                    ...
                    $cardata = array(
                        "available_date" => $avai_date->available_date
                    );
                }
    ...
    public function checkuserid($user_id)
    {
        ...
        $userdata = array();
        $userdata = $this->db->query("SELECT * FROM users WHERE id = $user_id")->result();
        if(!empty($userdata)){
