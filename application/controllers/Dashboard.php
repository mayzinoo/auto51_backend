...
class Dashboard extends CI_Controller
{
        ...
        {  
            // Insert
              ...
              if(!empty($post)) {
                  ...
                  $is_exist = $this->db->from("car_details")->where(['model_name' => $post["n_modelname"]])->count_all_results();
    ...
    function edit_vehicles()
    {
        ...
        {  
            ...
            $data["vehicledata"] = $this->db->query("SELECT car_details.id as vid,duration.duration as dura,duration.duration_id as duraid,car_package.best as beststatus,car_package.*,car_package.id as pkgid,car_package.duration as pkgduration,car_package.price as pkgprice,car_details.*,car_details.title_status as category,car_details.model_name as model_name,car_details.description as descrip,car_package.best as best FROM car_package LEFT JOIN car_details ON car_details.id = car_package.vehicle_id LEFT JOIN duration ON duration.duration_id=car_package.duration WHERE car_package.vehicle_id = $id")->row();

            $data["vehiclesdata"] = $this->db->query("SELECT * FROM car_details WHERE id = $id")->row();

            $data["vehiclespkgdata"] = $this->db->query("SELECT car_details.id as vid,duration.duration as dura,duration.duration_id as duraid,car_package.best as beststatus,car_package.*,car_package.id as pkgid,car_package.duration as pkgduration,car_package.price as pkgprice,car_details.*,car_details.title_status as category,car_details.model_name as model_name,car_details.description as descrip,car_package.best as best FROM car_package LEFT JOIN car_details ON car_details.id = car_package.vehicle_id LEFT JOIN duration ON duration.duration_id=car_package.duration WHERE car_package.vehicle_id = $id");
...
// Update    
      ...
      if($this->session->userdata("email") && $this->session->userdata("password"))
      {
          ...
          if($this->input->post('remove_photo')) // if remove photo checked
            ...
            if(!empty($_FILES['cover']['name']))
              ...
              redirect("dashboard/edit_vehicles/".$id);
      }else{
            redirect('/');
          }
    }
    function add_galleryphotos()
    {
        ...
        redirect("dashboard/edit_vehicles/".$vid);
    }
    ...
    function update_dealerapprove()
    {
        ...
        $config = Array(       
            ...
            $this->email->subject("Congratualtion ! - We approved you as dealer role with ".$mail);
...
// Delete
    ...
    function generateinvoicepdf(){
        ...
        $html = $this->load->view('invoicedownload',$data,true);
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
    public function sendinginvoice()
      ...
      $cvattach=$_FILES['cvattach']['tmp_name'];
        ...
        $this->email->to('auto51testing@gmail.com');
            ...
            if(!$is_exist) {
                      ...
                      { 
                        echo "<script>
                          ...
                          window.location.href='http://admin.auto51.com.sg/dashboard/users';
                          </script>"; 
                      }else{
                        $this->db->insert("users", $user);
                        $this->session->set_flashdata('success', 'User has been created.');
