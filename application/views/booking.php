            ...
            <div class="mdl-card__supporting-text no-padding">
                <table class="mdl-data-table mdl-js-data-table bordered-table">
                    ...
                    <tr>
                        ...
                        <th class="mdl-data-table__cell--non-numeric">Expiry Status</th>                   
                    ...
                    <tbody>
                      ...
                      ?>  
                            <tr>
                                ...
                                <?php if($row->leasing_status=="daily"){ ?>
                                    <td class="mdl-data-table__cell--non-numeric">
                                    <?php echo $row->total_days; ?> days <br/> SGD <?php echo $row->subscription_price; ?>/day  </td>
                                <?php }else{ ?>
                                    <td class="mdl-data-table__cell--non-numeric">
                                    <?php echo $row->pkgduration; ?> Months <br/> SGD <?php echo $row->subscription_price; ?>/month   </td>
                                <?php } ?>
                                
                                <td class="mdl-data-table__cell--non-numeric"><?php echo $row->start_date; ?></td>
                                <td class="mdl-data-table__cell--non-numeric"><?php echo $row->end_date; ?></td>
                                <?php if(date('Y-m-d', strtotime('+1 day', strtotime($row->end_date)))<=date('Y-m-d')){ ?>
                                    <td class="mdl-data-table__cell--non-numeric"><span style="color:red;font-weight: bold;">Expired</span></td>
                                <?php }else{ ?>
                                    <td></td>
                                <?php } ?>
                                

                                <td class="mdl-data-table__cell--non-numeric">SGD <?php echo $row->total_amt; ?></td>
                                ...
                                <td class="mdl-data-table__cell--non-numeric">
                                    ...
                                    <?php }else if($row->checkstatus=="2"){ ?>
                                        <b><span style="color:#f0ad4e">Waiting Approval</span></b>
                                ...
                                <?php }else if($row->checkstatus=="1"){ ?>
                                    <td>
                                       <a onclick="return confirm('Are you sure to reject?')" href="<?php echo base_url() .'dashboard/reject_bookingusers/'.$row->bid;?>" type="button" class="delete-asset btn btn-danger btn-xs color-white">
                                            Reject 
